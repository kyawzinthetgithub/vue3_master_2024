...
import { createNewTaskQuery, profilesQuery, projectsQuery } from '@/utils/supaQueriests'
...
<template>
  <Sheet v-model:open="sheetOpen">
    <SheetContent>
      ...
      <FormKit type="form" @submit="createTask" submit-label="Create Task" :config="{validationVisibility:'submit'}">
        ...
        <FormKit type="select" name="profile_id" id="profile_id" label="User" placeholder="Select a user"
          :options="selectOptions.profiles" validation="required" />
        ...
        <FormKit type="textarea" name="description" id="description" label="Description"
          placeholder="Task description" validation="length:0,255" />
      </FormKit>
    </SheetContent>
  </Sheet>
</template>
